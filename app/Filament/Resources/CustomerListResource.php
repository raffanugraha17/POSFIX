...
class CustomerListResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                TextInput::make('customer_telephone')->type('number')->required()->maxLength(200),
                TextInput::make('customer_age')->required()->integer(),
                TextInput::make('customer_occupation')->required(),
                TextInput::make('customer_address')->required(),
                Select::make('customer_gender')->required()->options([
                    'Male' => 'Male',
                    'Female' => 'Female',
                ]),
            ]);
    ...
    {
        // Log activity
        Log::info("Customer List created - ID: {$record->id}");
    }

    public static function afterUpdate($data, $record)
    {
        // Log activity
        Log::info("Customer List updated - ID: {$record->id}");
    }

    public static function afterDelete($record)
    {
        // Log activity
        Log::info("Customer List deleted - ID: {$record->id}");
    }
