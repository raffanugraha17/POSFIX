...
class CashierListResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                ...
                TextInput::make('cashier_telephone')->type('number')->required(),
                TextInput::make('cashier_role')->integer()->required(),
                Select::make('cashier_gender')->required()->options([
                    'Male' => 'Male',
                    'Female' => 'Female',
                ]),
                Select::make('cashier_shift')->required()->options([
                    'Morning' => 'Morning',
                    'Afternoon' => 'Afternoon',
                    'Night' => 'Night',
                ])
    ...
    {
        // Log activity
        Log::info("Cashier List created - ID: {$record->id}");
    }

    public static function afterUpdate($data, $record)
    {
        // Log activity
        Log::info("Cashier List updated - ID: {$record->id}");
    }

    public static function afterDelete($record)
    {
        // Log activity
        Log::info("Cashier List deleted - ID: {$record->id}");
    }
